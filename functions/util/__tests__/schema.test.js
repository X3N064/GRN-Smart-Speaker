...
describe('util - schema', function() {
  ...
  describe('defaultSchemas', function() {
    ...
    describe('SchemaType.URL_LIST', function() {
      const type = SchemaType.URL_LIST;
      const defaultVal = 'default url';
      const schema = {type, default: defaultVal};

      it('validates truthy values as uri links', function() {
        const links = ['https://www.google.com/', 'https://www.aol.com/'];
        expect(validator(links, schema)).to.eql(links);
      });

      it('fails invalided uri links', function() {
        const links = ['www.google.com/', 'https://www.aol.com/'];
        expect(validator(links, schema)).to.eql(defaultVal);
      });
    });
