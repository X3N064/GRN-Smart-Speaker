...
const validateSchemaType = (value, schema, label) => {
  ...
  try {
    ...
    if (validator.hasOwnProperty('default')) {
      logger.info(
        `Failed schema preprocess for ${label}, received: ${value}, use default: ${validator.default
        }`,
  ...
  if (result.error) {
    if (validator.hasOwnProperty('default')) {
      logger.info(
        `Failed schema validation for ${label}, received: ${value}, use default: ${validator.default
        }`,
